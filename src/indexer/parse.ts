...
export function html2text(html: string, type: 'docs' | 'blog' | 'page', url: string = '?') {
  ...
  if (type === 'docs') {
    ...
    $('span')
      .filter(
        (_, element) => $(element).hasClass('badge') && $(element).text().startsWith('Version:')
      )
      .remove();
  }
  ...
  if (type === 'docs' || type === 'blog') {
    ...
    const preFirstHeadingText = getText(
      $,
      $('article .markdown header, article .markdown h1').first().prevAll().get().slice().reverse()
    );
    ...
    $('article')
      ...
      .each((i, heading) => {
        ...
        if ($(heading).parents('.markdown').length === 0) {
          ...
          $sectionElements = $firstElement.nextUntil(`${HEADINGS}, header`).addBack();
        } else {
          ...
          const root = $(heading).parent('header').length ? $(heading).parent() : $(heading);
