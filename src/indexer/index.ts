...
import type { LoadContext, LoadedPlugin, OptionValidationContext, Plugin } from '@docusaurus/types';
...
export const tokenize = (input) => input.trim().toLowerCase()
  .split(${(lunrTokenizerSeparator ? lunrTokenizerSeparator : /[\s\-]+/).toString()})
  .filter(each => !!each);\n`;
...
export const tokenize = (input) => lunr.tokenizer(input)
  ...
  return {
    ...
    async postBuild({ routesPaths = [], outDir, baseUrl, siteConfig: { trailingSlash }, plugins }) {
          ...
          if (indexDocs) {
            for (const docsPlugin of docsPlugins.values()) {
              ...
              if (urlMatchesPrefix(route, docsBasePath)) {
                if (
                  urlMatchesPrefix(route, trimLeadingSlash(`${docsBasePath}/${docsTagsPath}`)) ||
      ...
      await Promise.all(
        Object.entries(documentsByDocusaurusTag).map(async ([docusaurusTag, documents]) => {
          ...
          const index = lunr(function () {
            ...
            const that = this;
            documents.forEach(
              ({ id, sectionTitle, sectionContent, sectionTags, docSidebarParentCategories }) => {
                let sidebarParentCategories;
                if (indexDocSidebarParentCategories > 0 && docSidebarParentCategories) {
              ...
              documents: documents.map(
                ({
                  ...
                  if (
                    ...
                    fullTitle = [...docSidebarParentCategories, pageTitle].join(' > ');
                  }
